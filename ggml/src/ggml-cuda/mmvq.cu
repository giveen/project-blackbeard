...
// Blackwell NVFP4 decode dispatch: uses vec_dot_nvfp4_q8_1_blackwell (VDR=8,
// fully unrolled sub-block processing) on sm120+. Definitions in vecdotq.cuh.
...
typedef float (*vec_dot_q_cuda_t)(const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs);
 ...
 static constexpr __device__ vec_dot_q_cuda_t get_vec_dot_q_cuda(ggml_type type) {
     switch (type) {
         ...
         case GGML_TYPE_NVFP4:   return vec_dot_nvfp4_q8_1_blackwell;
#else
...
#if defined(BLACKWELL_MMA_AVAILABLE)
         case GGML_TYPE_Q4_K:    return vec_dot_q4_K_q8_1_blackwell;
#else
...
#if defined(BLACKWELL_MMA_AVAILABLE)
        case GGML_TYPE_Q4_K:    return VDR_Q4_K_Q8_1_MMVQ_BLACKWELL;
#else
...
static constexpr __host__ __device__ int calc_rows_per_block(int ncols_dst, int table_id, bool small_k = false, int nwarps = 1) {
    ...
    if (table_id == MMVQ_PARAMETERS_BLACKWELL) {
        // Blackwell: rows_per_block=2 for decode to improve SM occupancy
        // (2 rows per block with nwarps=4 doubles work vs GENERIC's 1 row).
        // For ncols_dst>=2: rows_per_block=2 to match nwarps scaling.
        switch (ncols_dst) {
            case 1:
                return small_k ? nwarps : 2;  // 2 rows/block at decode
            case 2:
            case 3:
            case 4:
            case 5:
